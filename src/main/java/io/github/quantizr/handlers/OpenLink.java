...
public class OpenLink {
    ...
    public static void checkForLink(String type){
        ...
        if (AutoRoom.lastRoomHash == null) {
            player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED
                    + "FDR: You do not appear to be in a detected Dungeon room right now."));
            return;
        }
        if (AutoRoom.lastRoomJson == null) {
            player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED
                    + "FDR: This command does not work when the current room is detected as one of multiple."));
            return;
        }
        if (AutoRoom.lastRoomJson.get("dsg").getAsString().equals("null") && AutoRoom.lastRoomJson.get("sbp") == null) {
            player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED
                    + "FDR: There are no channels/images for this room."));
            return;
        }
        ...
        switch (type) {
            ...
            case "sbp":
                if (DungeonRooms.usingSBPSecrets) {
                    ...
                    player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED
                            + "FDR: You need the Skyblock Personalized (SBP) Mod for this feature, get it from ").appendSibling(sbp));
                }
                break;
        }
    ...
    public static void openDiscord(String type) {
        ...
        if (AutoRoom.lastRoomJson.get("dsg").getAsString().equals("null")) {
            player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED
                    + "FDR: There is no DSG channel for this room."));
            return;
        }
        try {
            if (type.equals("client")){
                player.addChatMessage(new ChatComponentText("FDR: Opening DSG Discord in Client..."));
                Desktop.getDesktop().browse(new URI("discord://" + AutoRoom.lastRoomJson.get("dsg").getAsString()));
            } else {
                player.addChatMessage(new ChatComponentText("FDR: Opening DSG Discord in Browser..."));
    ...
    public static void openSBPSecrets() {
        ...
        if (AutoRoom.lastRoomJson.get("sbp") == null) {
            player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED
                    + "FDR: There are no SBP images for this room."));
            return;
        }
