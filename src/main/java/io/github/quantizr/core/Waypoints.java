                            ...
                            if (pos.equals(event.pos)) {
                                for(int j = 1; j <= secretNum; j++) {
                                    if (secretsObject.get("secretName").getAsString().contains(String.valueOf(j))) {
                                        ...
                                        DungeonRooms.logger.info("FDR: Detected " + secretsObject.get("category").getAsString() + " click, turning off waypoint for secret #" + j);
                                        break;
                                    }
                                }
                            }
                                    ...
                                    if (entity.getDistanceSq(pos) <= 36D) {
                                        for(int j = 1; j <= secretNum; j++) {
                                            if (secretsObject.get("secretName").getAsString().contains(String.valueOf(j))) {
                                                ...
                                                Waypoints.secretsList.set(j-1, false);
                                                Waypoints.allSecretsMap.replace(AutoRoom.lastRoomName, Waypoints.secretsList);
                                                DungeonRooms.logger.info("FDR: " + entity.getCommandSenderEntity().getName() + " picked up " +  StringUtils.stripControlCodes(name) + " from a "  + secretsObject.get("category").getAsString() + " secret, turning off waypoint for secret #" + j);
                                                return;
                                            }
                                        }
                                    }
